...
void solve() {
    ...
    ll xuse = 0;
    ll closest = yuse*y;
    while(yuse>=0 and xuse>=0 and closest !=m){
        while(yuse*y+xuse*x<=m){
            xuse++;
        }
        if(yuse*y+xuse*x>m){
            xuse--;
        }
        ll curramount = yuse*y+xuse*x;

        closest = max(curramount,closest);
        //cout<<closest<<endl<<xuse<<endl<<yuse<<endl;
        //cout<<endl;
        yuse--;
        while(yuse*y+xuse*x<=m){
            xuse++;
